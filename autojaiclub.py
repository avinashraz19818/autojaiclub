...
class WinGoBotEnhanced:
    def __init__(self, bot_token, api_id=None, api_hash=None, phone=None):
        ...
        # Global prediction schedule: 06:00-00:00, 50min active + 10min break
        self.prediction_window_start = 6
        self.prediction_window_end = 24
        self.prediction_active_minutes = 50
        self.prediction_break_minutes = 10
    ...
    def is_channel_in_time_window(self, channel_id=None, dt=None):
        if dt is None:
            dt = self.get_ist_now()
        h = dt.hour
        m = dt.minute
        if h < self.prediction_window_start or h >= self.prediction_window_end:
            return False
        return m < self.prediction_active_minutes

    def get_minutes_until_session_start(self, channel_id=None, dt=None):
        if dt is None:
            dt = self.get_ist_now()
        h = dt.hour
        m = dt.minute
        if h < self.prediction_window_start:
            return (self.prediction_window_start - h) * 60 - m
        if h >= self.prediction_window_end:
            return (24 - h + self.prediction_window_start) * 60 - m
        if m < self.prediction_active_minutes:
            return 0
        return 60 - m

    def get_current_session_name_for_channel(self, channel_id=None, dt=None):
        if dt is None:
            dt = self.get_ist_now()
        hour = dt.hour
        start = f"{hour:02d}:00"
    ...
    def get_next_session_time_for_channel(self, channel_id=None, dt=None):
        if dt is None:
            dt = self.get_ist_now()
        h = dt.hour
        m = dt.minute
        if h < self.prediction_window_start:
            return f"{self.prediction_window_start:02d}:00"
        if h >= self.prediction_window_end:
            return f"{self.prediction_window_start:02d}:00"
        if m < self.prediction_active_minutes:
            return f"{h:02d}:{self.prediction_active_minutes:02d}"
        nh = h + 1
        if nh >= self.prediction_window_end:
            return "Tomorrow 06:00"
        return f"{nh:02d}:00"

    def is_after_last_session_for_channel(self, channel_id=None, dt=None):
        if dt is None:
            dt = self.get_ist_now()
        return dt.hour >= self.prediction_window_end
    ...
    def ensure_emoji_config_keys(self):
        ...
        self.emoji_config.setdefault('premium_emojis', {}).setdefault('fire1', '<emoji id=5402406965252989103>🔥</emoji>')
        self.emoji_config.setdefault('premium_emojis', {}).setdefault('alarm1', '<emoji id=5368295871131695793>⏰</emoji>')
        self.emoji_config.setdefault('premium_emojis', {}).setdefault('tick', '<emoji id=5208893571599449245>✅</emoji>')
        self.emoji_config.setdefault('premium_emojis', {}).setdefault('rarrow', '<emoji id=6127194666526841786>➡️</emoji>')
        self.emoji_config.setdefault('regular_emojis', {}).setdefault('fire1', '🔥')
        self.emoji_config.setdefault('regular_emojis', {}).setdefault('alarm1', '⏰')
        self.emoji_config.setdefault('regular_emojis', {}).setdefault('tick', '✅')
        self.emoji_config.setdefault('regular_emojis', {}).setdefault('rarrow', '➡️')
        self.emoji_config.setdefault('emoji_to_placeholder', {}).setdefault('🔥', '{fire1}')
        self.emoji_config.setdefault('emoji_to_placeholder', {}).setdefault('⏰', '{alarm1}')
        self.emoji_config.setdefault('emoji_to_placeholder', {}).setdefault('✅', '{tick}')
        self.emoji_config.setdefault('emoji_to_placeholder', {}).setdefault('➡️', '{rarrow}')
        self.emoji_config.setdefault('placeholder_to_emoji', {}).setdefault('{fire1}', '🔥')
        self.emoji_config.setdefault('placeholder_to_emoji', {}).setdefault('{alarm1}', '⏰')
        self.emoji_config.setdefault('placeholder_to_emoji', {}).setdefault('{tick}', '✅')
        self.emoji_config.setdefault('placeholder_to_emoji', {}).setdefault('{rarrow}', '➡️')
    ...
    def format_single_prediction(self, channel_id, period, prediction, for_channel=False):
        ...
        format_dict = {
            ...
            'period': period_text,
            'session': self.get_current_session_name_for_channel(channel_id),
...
• {'✅ Confidence is high' if getattr(self, 'last_ai_confidence', 0) > 0.7 else '⚠️ Low confidence predictions'}"""
                ...
                await query.edit_message_text(
                    f"⏰ Schedule Info for {channel_id}\n\n"
                    f"Current Schedule:\n{current_text}\n\n"
...
• Break: {self.get_channel_template(channel_id, 'break_message')[:50]}..."""
    ...
    def get_keyboard(self, keyboard_type, channel_id=None, message_index=None, result_type=None, event_type=None):
        ...
        main_menu = [
            ...
            [InlineKeyboardButton("⏰ Schedule Info", callback_data="view_time_windows")]
        ]
        ...
        if keyboard_type == 'channel_config' and channel_id:
            ...
            channel_config_menu = [
                ...
                [InlineKeyboardButton("⏰ Schedule Info", callback_data=f"set_time_windows:{channel_id}")],
